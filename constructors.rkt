...
;; Definim algoritmul de construcție a unui
...
; Funcție recursiva pe stiva care primește un text (listă 
...
; Funcție implementata cu functionale care primește o listă de cuvinte 
; și un caracter ch și întoarce acele cuvinte din listă care 
; încep cu caracterul ch.
(define (starts-with? ch)
	(lambda (word)
		(if (null? word)
			false
			(char=? (car word) ch)
...
(define (get-ch-words words ch)
	(filter (starts-with? ch) words)
)


; Funcție nerecursiva explicit care primește o listă nevidă de sufixe 
...
(define (ast-func suffixes)
	(append (list (list (caar suffixes)))
		(map (lambda (s) (drop s 1)) suffixes))
)
...
(define (cst-func suffixes)
	(define common-prefix (longest-common-prefix-of-list suffixes))
	(append (list common-prefix)
		(map (lambda (s) (drop s (length common-prefix))) suffixes))
)


; Funcția suffixes->st care construiește un
...
(define (to-fmt labeling-func suffix)
	(if (> (length (car suffix)) 1)
		(let* ((split-result (labeling-func suffix))
			   (recursive-result (to-fmt labeling-func (cdr split-result))))
			(if (not (null? (car recursive-result)))
				(cons (car split-result) (list recursive-result))
				(list (car split-result))
			)
		)
		suffix
	)
...
(define (recursive-step labeling-func new-suffixes alphabet)
	(if (>= (length new-suffixes) 2)
		(let* ((pair-result (labeling-func new-suffixes))
			   (current-label (car pair-result))
			   (not-null-new-suffixes (filter (lambda (suff) (not (null? suff))) (cdr pair-result)))
			  )
			  (cons current-label (suffixes->st labeling-func not-null-new-suffixes alphabet))
		)
		(to-fmt labeling-func new-suffixes)
	)
)

(define (suffixes->st labeling-func suffixes alphabet)
	(if (>= (length (car suffixes)) 1)
		(let* ((grouped-suffixes (map (lambda (ch) (get-ch-words suffixes ch)) alphabet))
			   (not-null-grouped-suffixes (filter (lambda (suff) (not (null? suff))) grouped-suffixes))
			  )
			  (map (lambda (group) (recursive-step labeling-func group alphabet)) not-null-grouped-suffixes)
		)
		suffixes
	)
)
...
; a) Funcția text->st care primește un text
...
(define (get-alphabet text)
	(append (sort (remove-duplicates text) char<?))
)

(define text->st
	(lambda (labeling-func)
		(lambda (text)
			(let* ((ended-text (append text '(#\$)))
				   (suffixes (get-suffixes ended-text))
				   (alphabet (get-alphabet ended-text))
				  )
...
; b) Din funcția text->st se deriveaza funcția text->ast care
; primește un text (listă de caractere) și întoarce AST-ul
; asociat textului.
(define text->ast (text->st ast-func))

; c) Din funcția text->st se deriveaza funcția text->cst care
